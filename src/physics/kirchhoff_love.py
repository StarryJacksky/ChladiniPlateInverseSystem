...
def score_thickness_with_kl_proxy(H_mm: np.ndarray, target_grid: np.ndarray, config: dict, num_modes: int = 8) -> dict:  # 使用 KL 离散特征方程评分厚度场 / Score thickness field with KL discrete eigen equation
    modes = solve_kl_modes(H_mm, float(config["project"]["plate_length_mm"]), float(config["project"]["plate_width_mm"]), config.get("material", {}), num_modes=num_modes)  # 求解代理模态 / Solve proxy modes
    target = resize_target_to_grid(target_grid, H_mm.shape[0])  # 对齐目标网格尺寸 / Align target grid shape
    ...
    return {"kl_proxy_score": float(scores[best_index] if scores else 0.0), "kl_proxy_mode": best_index + 1}  # 返回代理评分结果 / Return proxy score result
