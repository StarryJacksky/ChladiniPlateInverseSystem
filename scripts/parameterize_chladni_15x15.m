function parameterize_chladni_15x15() % 执行本行 MATLAB/COMSOL 操作 / Execute this MATLAB/COMSOL operation
%PARAMETERIZE_CHLADNI_15X15 Add the calibrated 15x15 optimisation parameters. / 中文说明：本注释解释脚本意图 / Chinese note: this comment explains the script intent
...
output_path = '/Users/apple/Documents/ChladiniPlateProject/comsol_templates/Chladni_15x15_parameterized.mph'; % 执行本行 MATLAB/COMSOL 操作 / Execute this MATLAB/COMSOL operation
...
for row = 1:grid_size % 执行本行 MATLAB/COMSOL 操作 / Execute this MATLAB/COMSOL operation
    for col = 1:grid_size % 执行本行 MATLAB/COMSOL 操作 / Execute this MATLAB/COMSOL operation
        ...
        model.param.descr(name, sprintf('15x15 thickness cell (%d,%d) / 15x15 厚度单元 (%d,%d)', row, col, row, col));
