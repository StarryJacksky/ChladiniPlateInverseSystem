"""Probe the bound COMSOL model structure (physics/material/coordSystems).

Outputs a plain text log into data/comsol_exports/probe_model_log.txt for the
forthcoming orthotropic shell modification.
"""
...
def main() -> None:
    ...
    output_log = Path("data/comsol_exports/probe_model_log.txt").resolve()
    ...
    parts.append(
        f"probe_model_structure({matlab_quote(model.resolve())},{matlab_quote(output_log)})"
    )
    batch = "; ".join(parts)
    command = [matlab, "-nosplash", "-noFigureWindows", "-sd", str(Path.cwd()), "-batch", batch]
    log_path = Path("data/comsol_exports/probe_run.log")
