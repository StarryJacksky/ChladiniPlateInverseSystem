"""Probe COMSOL shell.emm1 orthotropic property + shellsys rotation API."""
...
def main() -> None:
    ...
    output_log = Path("data/comsol_logs/orthotropic_emm1_probe.log").resolve()
    ...
    parts.append(
        f"probe_orthotropic_emm1({matlab_quote(model.resolve())},{matlab_quote(output_log)})"
    )
    batch = "; ".join(parts)
    command = [matlab, "-nosplash", "-noFigureWindows", "-sd", str(Path.cwd()), "-batch", batch]
    log_path = Path("data/comsol_logs/probe_orthotropic_emm1_run.log")
